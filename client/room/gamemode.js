/*
Ку ТНТ. Я тут немного сделал, если не подходит что-то скажи, уберу, ну или сам убери. За код не бойся, он будет после этого текста, ниже.
Так как ты не любишь когда я тебя заставляю учиться, то вот просто материал, а ты уж сам думай (пишу мини обучение по JS, если захочешь прочитай, никто не заставляет, можешь просто удалить этот текст):
...
Вот самое простое что можешь сделать с числами: +, -, *, /, ** (степень), % (остаток от деления).

Если говорить о строках, то они могут быть из трёх типов кавычек: ', ", и `.
...
'строка'.indexOf('о') // даёт 3, так как 'о' находится на 4 месте (всегда уменьшено на 1).
#E
...
Есть два варианта сравнения: == и ===. Первый сравнивает нестрого, второй строго (также сравнивает типы),
вот например: '2' == 2 даст true, но если использовать === то нет.
...
// Например в PC2 можно сделать такю функцию:
function ph(p, h) {
        p.Ui.Hint.Value = h;
}
// И потом делать так:
...
Ну а дальше идёт твой код, который, надеюсь я исправил...
*/
